...
createConnection().then(connection => {
        ...
        [
            {
                client_id: '123',
                client_secret: '123',
                redirect_uris: ['http://localhost:1000/home'],
    ...
    oidc.initialize({clients}).then(() => {
        ...
        oidcApi.listen(PORT, () => {
            console.log(`oidc-provider listening on port ${PORT}, check http://localhost:${PORT}/oauth/auth?client_id=${clients[0].client_id}&redirect_uri=${clients[0].redirect_uris}&scope=openid&response_type=code&state=12&nonce=123`);
        });
    });
